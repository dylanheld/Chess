...
vector<Coord> Pawn::potentialMoves(Coord stc){
  ...
  if (colour == 1) dir = -1; //ASSUMPTION: White is always starting at the bottom

  //Checking potential vertical moves
  //cout << "apple; x " << stc.addCoord(0, dir*1).getX() << " " << stc.addCoord(0, dir*1).getY();
  ...
  //Next two for attacking diagonal
  ...
  for (int coord = 1;;) {
	...
	//cout << "Checking testCoord: " << testCoord.getX() << " " << testCoord.getY() << endl;
	char pieceIQ = 'p';
	if (colour == 1) { pieceIQ = 'P'; }
