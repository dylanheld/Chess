...
bool checkBoard(GameBoard *G) {
	...
	if(numWK != 1 || numBK != 1) { return false; }
	else {
		...
		if(temp->wouldPutInCheck(WkCoord, WkCoord) || temp2->wouldPutInCheck(BkCoord, BkCoord)) { 
			...
			return false; }
		else { delete temp; delete temp2; return true; }
	}
}
	...
	//cout << "entering setup mode" << endl;
		...
		if(index == 0) {
			if (inputCommand == "done") {
				if(checkBoard(G)) { break; }
				else { cerr << "Cannot leave setup mode, board is presently invalid" << endl; }
                        }
