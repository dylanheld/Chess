...
void AI4::makeMove() {
  ...
  while(cin >> canMove) {
    ...
    if (canMove == "move") {
      ...
      if (!allMoves.empty()) { cerr << "AI3: Line around 50 - impossible situaiton occured"; }
	...
	for(int Attack = 2; Attack >= -3; Attack--) {
		...
		for(it_type iteratorA = opponentMoves.begin(); iteratorA != opponentMoves.end(); iteratorA++) {
		      ...
		      break;
				...
				if(it->getX() == Attacker.getX() && it->getY() == Attacker.getY()) {
					...
					if ((myP == 'q' && tp == 'q') || (myP == 'r' && (tp == 'r' || tp == 'q')) || (myP == 'b' && (tp == 'b' || tp == 'r' || tp == 'q')) || (myP == 'n' && (tp == 'n' || tp == 'b' || tp == 'r' || tp == 'q')) || myP == 'p') {
						++it;
					}
					else {
						it = v.erase(it);
		...
		else if(Attack == 1) { //AKA we are defending
                //SEE IF CURRENT LOCATION IS BEING ATTACKED
                bool isAttacked = false;
                typedef map<Coord*, vector<Coord> >::iterator it_type;
                for(it_type iteratorA = opponentMoves.begin(); iteratorA != opponentMoves.end(); iteratorA++) {
              ...
              //CODE FOR ATTACK = 1, ATTACK = 0      
                ...
                else if ((Attack == 0 || Attack == -2) && (gb->getPiece(*it) == NULL) && !(wouldPutInCheck(temp, *it, true))) {
                        it = v.erase(it);
